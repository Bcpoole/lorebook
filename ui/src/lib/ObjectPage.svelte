<script>
  ...
  async function loadObject() {
    ...
    try {
      const res = await fetch(`/api/artifact/object/${id}`)
      if (res.status === 404) {
      ...
      error = 'Error loading object: ' + (err.message || 'Unknown error')
    } finally {
      loading = false
    }
  }
  ...
  async function handleSave(updatedData) {
    ...
    object = { ...object, ...updatedData.artifact }
    editMode = false
    toastMessage = 'Object updated ✓'
    toastVisible = true
  }
  ...
  async function handleDelete() {
    ...
    deleting = true
    try {
      const res = await fetch(`/api/artifact/object/${object.id}`, {
        method: 'DELETE',
      })
      ...
      toastMessage = 'Object deleted ✓'
      toastVisible = true
      setTimeout(() => {
        window.location.href = '/worlds-stories'
      }, 1500)
    } catch (err) {
      error = err.message || 'Failed to delete object'
    } finally {
      deleting = false
    }
  }
  ...
  {:else if object}
      ...
      <div class="object-display">
        ...
        {#if object.image_file}
          <div class="image-container">
            <img src={object.image_data || object.image_file} alt={object.name} />
          </div>
        {/if}
        ...
        {#if object.material}
          <section class="section">
            <h3>Material</h3>
            <p><strong>{object.material}</strong></p>
          </section>
        {/if}
        ...
        <div class="metadata">
          <p>
            <small>ID: <code>{object.id}</code></small>
          </p>
          {#if object.created_at}
            <p>
              <small>Created: {new Date(object.created_at).toLocaleString()}</small>
            </p>
          {/if}
          {#if object.modified_at}
            <p>
              <small>Modified: {new Date(object.modified_at).toLocaleString()}</small>
            </p>
          {/if}
        </div>
      </div>
    {:else}
      <div class="edit-container">
        <ObjectEditPanel object={object} onSave={handleSave} onCancel={() => (editMode = false)} />
      </div>
    {/if}
  {/if}
